...
use App\Models\SubmitLoanDocs as ModelsSubmitLoanDocs;
...
class SubmitLoanDocs implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;
    private $loanLeadDocs;
     ...
     * @return void
     */
    public function __construct(ModelsSubmitLoanDocs $loanDocs)
    {
        $this->loanLeadDocs = $loanDocs;
    }
    ...
    {
        $loanLeadDocs = ModelsSubmitLoanDocs::whereId($this->loanLeadDocs->id)->first();
        $loanLead       = LoanLead::whereId($loanLeadDocs->loan_lead_id)->first();
        $loanLeadDetail = LoanLeadDetail::where('loan_lead_id', $loanLead->id)->first();

        //CustomLeadLog::create(['loan_lead_id' => $loanLead->id, 'exception' => "$loanLeadDocs->id - $loanLead->id - $loanLeadDetail->id", 'type' => '$type', 'status'=>'failed']);
        //die;
       ...
       $customFields['PFuPGYujpR4mdlqpoRLY'] = $loanLeadDetail->eligibility_result;
        ...
        //docs
        //    $customFields['qrsE3aB0RzEJK6OQdVwY'] = $loanLeadDocs->loan_doc_ssn;
        //    $customFields['DQVYVyn7W4zkITLw5YeX'] = $loanLeadDocs->loan_doc_tax_id_number;
        $customFields['hKk5QVyZaBn8eOzWVX13'] = $loanLeadDocs->file_dir;
        //    $customFields['nXq9erzsa652qB3hOTHd'] = $loanLeadDocs->loan_doc_03_months_bank_statment;
        //    $customFields['3PYvBvGVcDaqIrhG22nP'] = $loanLeadDocs->loan_doc_collateral;
       ...
       $customTags[]        = 'bcc_lead_calculator_document_submission';
       ...
       if (count($fieldsData)) {
           ...
           // return $response;
           if (isset($status)) {
               CustomLeadLog::create(['loan_lead_id' => $loanLead->id, 'exception' => $exception, 'type' => 'doc_update_ghl', 'status'=>$status]);
           }
       }
       ...
       //update HubSpot deal
        ...
        $arr = array(
            'properties' => array(
                ...
                array(
                    'property' => 'laon_doc___signature',
                    'value' => $loanLeadDocs->file_dir
                )
                ,
        ...
        $hubSpotContactId = $loanLead->hubspot_contact_id;
        $endpoint = "https://api.hubapi.com/contacts/v1/contact/vid/".$hubSpotContactId."/profile?hapikey=" . env('HUBSPOT_API_KEY');
        $ch = @curl_init();
        ...
        //CustomLeadLog::create(['loan_lead_id' => $loanLead->id, 'exception' => $response, 'type' => 'hub', 'status'=>'failed']);
