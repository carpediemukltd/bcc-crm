<div id="list">
    <h6 class="mb-1">Previous Notes: (<span id="show_previous_notes">{{ count($notes) }}</span>)</h6>
    @if (count($notes) == 0)
        <p id="no_notes_found">No notes found.</p>
    @else
        @foreach ($notes as $note)
            <div class="nav-item mb-3 p-3" style="border: 1px solid #eee; border-radius:3px; ">
                ...
                @if ($note->user_id == Auth()->user()->id)
                    <div id="note_save_rights_{{ $note->id }}" style="display:none;float: right;">

                        <a href="javascript:void(0)"
                            onclick="cancelEdit('{{ $note->id }}','{{ $note->user_id }}');">
                        ...
                        <a href="javascript:void(0)"
                            onclick="saveEditNote('{{ $note->id }}','{{ $note->user_id }}');">
                            <svg fill="none" xmlns="http://www.w3.org/2000/svg" class="icon-32" width="32"
                    ...
                    <div id="note_rights_{{ $note->id }}" style="float: right;">

                        <a href="javascript:void(0)"
                            onclick="deleteNote('{{ $note->id }}','{{ $note->user_id }}');">
                            <svg fill="none" xmlns="http://www.w3.org/2000/svg" width="32" height="32"
                                viewBox="0 0 32 32">
                                <path fill-rule="evenodd" clip-rule="evenodd"
                        ...
                        <a href="javascript:void(0)"
                            onclick="showEditNote('{{ $note->id }}','{{ $note->user_id }}');">
                            <svg fill="none" xmlns="http://www.w3.org/2000/svg" class="icon-32" width="32"
                                height="32" viewBox="0 0 32 32">
                                <path fill-rule="evenodd" clip-rule="evenodd"
                ...
                @endif
                <br />
                <b>By:</b> <span>{{ $note->first_name }} {{ $note->last_name }}</span>
                (<span>{{ $note->role }}</span>)
                <b>at:</b> <span>{{ $note->created_at }}</span>
                @if($note->created_at!=$note->updated_at)
                <br /><b>Edited:</b> <span>{{ $note->created_at }}</span>
                @endif
                <br />
            </div>
        @endforeach
