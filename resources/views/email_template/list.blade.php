                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            <div class="header-title">
                                <h4 class="card-title">List of Email Templates</h4>
                            </div>
                        </div>
                            ...
                            <div class="table-responsive">
                                <div id="user-list-table_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                                    <div class="table-responsive my-3">
                                        <table id="user-list-table" class="table email_template_holder table-striped dataTable no-footer"
                                            role="grid" data-toggle="" aria-describedby="user-list-table_info">
                                            <thead>
                                                <tr class="ligth">
                                                    <th width="100%" class="sorting" tabindex="0"
                                                        aria-controls="user-list-table">Subject / Body</th>
                                                        ...
                                                        <tr class="odd" id="{{ $rec->id }}">
                                                            ...
                                                            <td class="text-center">
                                                                <div class="d-flex justify-content">
                                                                    <div id="edit_rights_{{ $rec->id }}">
                                                                        <a href="javascript:void(0)"
                                                                            onclick="showEditOption('{{ $rec->id }}');">
                                                                            <svg fill="none"
                                                                                xmlns="http://www.w3.org/2000/svg"
                                                                                class="icon-32" width="32"
                                                                        ...
                                                                        <a href="javascript:void(0)"
                                                                            onclick="DeleteConfirm('{{ $rec->id }}','{{ $rec->subject }}');">
                                                                            <svg fill="none"
                                                                                xmlns="http://www.w3.org/2000/svg"
                                                                                class="icon-32" width="32"
                                                                                height="32" viewBox="0 0 32 32">
    ...
    <script type="text/javascript">
        ...
        function saveEdited(id) {
            ...
            if (subject !== '' && body !== '') {
                ...
                $('#loading_' + id).show();
                var url = "{{ route('email_template.edit', ':data_id') }}";
                ...
                $.post({
                    ...
                    success: function(res) {
                        $('#show_text_' + id).html(res.subject);
                        $('#show_text2_' + id).html(res.body);
                        $('#show_text_' + id).show();
                        $('#show_text2_' + id).show();
                        $('#data_subject_' + id).val(res.subject);
                        $('#data_body_' + id).val(res.body);
                        $('#show_edit_text_' + id).hide();
                        $('#edit_rights_' + id).show();
                        $('#save_rights_' + id).hide();
                        $('#loading_' + id).hide();
        ...
        function saveNew(id) {
            ...
            if (subject !== '' && body !== '') {
                $('#loading_' + id).html($('#show_loading').html());
                $('#loading_' + id).show();
                var url = "{{ route('email_template.add') }}";
                $.post({
                    url: url,
                    type: 'POST',
                    data: {
        ...
        $('.addClickrBtn').click(function() {
            ...
            var tr = '<tr id="' + c + '"><td><div><input type="text" placeholder="New Subject" id="data_subject_' + c +
                ...
                '" style="display: none;"></div></td><td><div class="d-flex">';
            tr += '<a href="javascript:void(0)" class="save_new" onclick="saveNew(' + c + ');">';
            tr +=
        ...
        function DeleteConfirm(id,subject) {
            ...
            if (r) {
                $('#loading_' + id).html($('#show_loading').html());
                $('#loading_' + id).show();
                var url = "{{ route('email_template.delete', ':email_template_id') }}";
                url = url.replace(':email_template', id);
                $.post({
                    url: url,
                    type: 'POST',
                    data: {
